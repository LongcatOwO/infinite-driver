...
namespace infd {
	...
	void ShaderBuilder::setShader(GLenum type, const std::string &filename) {
		...
		try {
            setShaderSource(type, buffer.str());
		}
	...
	void ShaderBuilder::setShaderSource(GLenum type, const std::string &shadersource) {

		// same as GLint shader = glCreateShader(type);
		GLShader shader {static_cast<GLShaderType>(type)};
